...
from third_set.sofascore import (
    ...
    discover_player_profile_urls_from_match_diagnostic,
)
from third_set.stats_parser import sum_event_value
from third_set.dom_stats import DomStatsError, extract_statistics_dom
from third_set.analyzer_core.logging_utils import _dbg, _log_step, _percentile
...
class HistoryAudit:
    ...
    source_links: int = 0
    source_api_ok: bool = True
    recovery_pass_used: bool = False
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "source_api_ok": self.source_api_ok,
            "recovery_pass_used": self.recovery_pass_used,
        }


def _history_source_unavailable(
    *,
    links_count: int,
    ...
    if int(valid) > 0:
        return False
    if int(links_count) > 0:
        return False
    if int(candidates) > 0 and int(api_history_fetch_failed) <= 0:
        return False
    return int(api_history_fetch_failed) > 0 or int(candidates) <= 0
    ...
    *,
    ...
    reset_codes_raw = os.getenv(
        ...
        (
            ...
            "profile_links_empty,api_history_fetch_failed,no_finished_singles,history_source_unavailable"
        ),
    )
    ...
    async def _get_api_events_with_fallback(api_page: Page, *, api_needed: int, api_timeout: float) -> Tuple[List[Dict[str, Any]], bool]:
        try:
            if no_limits_enabled and slow_mode:
                got = await get_last_finished_singles_events(api_page, int(team_id), limit=min(200, int(api_needed)))
            else:
                got = await asyncio.wait_for(
                    get_last_finished_singles_events(api_page, int(team_id), limit=min(200, int(api_needed))),
                    timeout=api_timeout,
                )
            return list(got or []), True
        except Exception:
            pass
        try:
            nav_page_index = 0
            ...
            fetched_any = False
            while len(picked) < int(api_needed):
                if no_limits_enabled and slow_mode:
                    payload = await get_team_last_events_via_navigation(
                        api_page, int(team_id), page_index=nav_page_index, timeout_ms=20_000
                    )
                else:
                    payload = await asyncio.wait_for(
                        get_team_last_events_via_navigation(
                            api_page, int(team_id), page_index=nav_page_index, timeout_ms=15_000
                        ),
                        timeout=max(2.0, min(15.0, api_timeout)),
                    )
                fetched_any = True
                picked.extend(
                    pick_last_finished_singles_events(payload, limit=max(0, int(api_needed) - len(picked)))
                )
                if not bool((payload or {}).get("hasNextPage")):
                    break
                nav_page_index += 1
                if nav_page_index >= 6:
                    break
            return picked[: int(api_needed)], fetched_any
        except Exception:
            return [], False
    ...
    if match_url and match_event_id and match_side in ("home", "away"):
        ...
        if prof:
            ...
            try:
                if no_limits_enabled and slow_mode:
                    prof, profile_diag = await discover_player_profile_urls_from_match_diagnostic(
                        page, match_url=str(match_url), event_id=int(match_event_id)
                    )
                else:
                    prof, profile_diag = await asyncio.wait_for(
                        discover_player_profile_urls_from_match_diagnostic(
                            page, match_url=str(match_url), event_id=int(match_event_id)
                        ),
        ...
        if isinstance(profile_diag, dict) and profile_diag:
            _log_step(
                f"History[{progress_label or match_side}]: profile_diag "
                f"strategy={profile_diag.get('strategy')} vis={profile_diag.get('visible_anchors')} "
                f"hdr={profile_diag.get('header_anchors')} retry={int(bool(profile_diag.get('retry_used')))}"
            )
    ...
    if len(events) < max(max_history * 2, scan_initial_cfg):
        try:
            ...
            api_events, source_api_ok = await _get_api_events_with_fallback(
                page, api_needed=min(200, int(api_needed)), api_timeout=api_timeout
            )
        except Exception:
            api_events = []
            source_api_ok = False
        if not source_api_ok:
        ...
        for ev in api_events or []:
            ...
            if len(events) >= candidate_target:
                break
    if _history_source_unavailable(
        links_count=len(links),
        api_history_fetch_failed=int(excluded_by_reason.get("api_history_fetch_failed", 0) or 0),
        candidates=len(events),
        valid=0,
    ):
        recovery_pass_used = True
        _log_step(
            f"History[{progress_label or match_side}]: source_recovery start "
            f"links={len(links)} candidates={len(events)} api_ok={int(bool(source_api_ok))}"
        )
        recovery_page: Optional[Page] = None
        try:
            recovery_page = await page.context.new_page()
            recovery_links: List[str] = []
            if match_url and match_event_id and match_side in ("home", "away"):
                rec_prof, rec_diag = await discover_player_profile_urls_from_match_diagnostic(
                    recovery_page, match_url=str(match_url), event_id=int(match_event_id)
                )
                profile_diag = dict(profile_diag or {})
                profile_diag["recovery"] = rec_diag
                rec_profile_url = rec_prof.get(match_side) if isinstance(rec_prof, dict) else None
                if isinstance(rec_profile_url, str) and rec_profile_url:
                    try:
                        recovery_links = await discover_player_match_links_from_profile_url(
                            recovery_page, profile_url=rec_profile_url, limit=link_limit
                        )
                    except Exception:
                        recovery_links = []
            if not recovery_links and isinstance(team_slug, str) and team_slug.strip():
                try:
                    recovery_links = await discover_player_match_links(
                        recovery_page, team_id=int(team_id), team_slug=str(team_slug), limit=link_limit
                    )
                except Exception:
                    recovery_links = []
            if recovery_links:
                links = recovery_links
                excluded_by_reason.pop("profile_links_empty", None)
                for link in recovery_links:
                    if loop.time() >= candidate_deadline:
                        break
                    if len(events) >= candidate_target:
                        break
                    try:
                        if slow_mode:
                            _accept_candidate_link(str(link), source="profile_links_recovery")
                        else:
                            ev = await _resolve_event_from_link(str(link), resolve_page=recovery_page)
                            if ev is None:
                                continue
                            _accept_candidate_event(ev)
                    except Exception:
            ...
            if len(events) < max(max_history * 2, scan_initial_cfg):
                ...
                api_events, source_api_ok = await _get_api_events_with_fallback(
                    recovery_page,
                    api_needed=min(200, int(api_needed)),
                    api_timeout=max(3.0, min(18.0, candidate_build_budget_s)),
                )
                if source_api_ok:
                    excluded_by_reason.pop("api_history_fetch_failed", None)
                else:
                    excluded_by_reason["api_history_fetch_failed"] = excluded_by_reason.get(
            ...
            if recovery_page is not None:
                try:
                    ...
                    pass
    source_unavailable_final = _history_source_unavailable(
        links_count=len(links),
        api_history_fetch_failed=int(excluded_by_reason.get("api_history_fetch_failed", 0) or 0),
        candidates=len(events),
        valid=0,
    )
    if source_unavailable_final:
        excluded_by_reason["history_source_unavailable"] = excluded_by_reason.get("history_source_unavailable", 0) + 1
        excluded_by_reason.pop("profile_links_empty", None)
        excluded_by_reason.pop("no_finished_singles", None)
    candidate_discovery_ms = max(0.0, (loop.time() - candidate_discovery_started) * 1000.0)
    source_health = (
        "unavailable"
        if source_unavailable_final
        else ("degraded" if (not source_api_ok or len(links) <= 0) else "ok")
    )
    _log_step(
        f"History[{progress_label or match_side}]: candidate_events={len(events)} target={candidate_target} "
        f"source_health={source_health} links={len(links)} api_ok={int(bool(source_api_ok))} "
        f"recovery_pass_used={int(bool(recovery_pass_used))}"
    )
        ...
        for t in done:
            ...
            if reason is not None:
                ...
                if reason == "dom_stats_error":
                    ...
                    last_ui_unavailable_code = None
                excluded_by_reason[reason] = excluded_by_reason.get(reason, 0) + 1
                if workers == 1 and consecutive_source_failures >= 2:
                    await _reset_single_page("history_source_unavailable")
                    consecutive_source_failures = 0
    ...
    if len(events) == 0 and not source_unavailable_final:
    ...
    audit = HistoryAudit(
        ...
        source_links=int(len(links)),
        source_api_ok=bool(source_api_ok),
        recovery_pass_used=bool(recovery_pass_used),
    )
    try:
        for p in hist_pages:
            await p.close()
    ...
    *,
    ...
    def _home_unavailable_for_fair_compare(rows: List[Dict[str, Any]], audit: HistoryAudit) -> bool:
        ...
        if ff in ("budget_exceeded", "budget_exceeded_wait"):
            ...
            hard = hard or int(exr.get("history_source_unavailable", 0) or 0) > 0
    ...
    _log_step(
        ...
        f"home={getattr(audit_home, 'source_health', 'ok')} links={int(getattr(audit_home, 'source_links', 0) or 0)} "
        f"api_ok={int(bool(getattr(audit_home, 'source_api_ok', True)))} "
        f"recovery={int(bool(getattr(audit_home, 'recovery_pass_used', False)))} "
        f"resets={int(getattr(audit_home, 'context_resets', 0) or 0)} | "
        f"away={getattr(audit_away, 'source_health', 'ok')} links={int(getattr(audit_away, 'source_links', 0) or 0)} "
        f"api_ok={int(bool(getattr(audit_away, 'source_api_ok', True)))} "
        f"recovery={int(bool(getattr(audit_away, 'recovery_pass_used', False)))} "
        f"resets={int(getattr(audit_away, 'context_resets', 0) or 0)}"
    )
    meta_diag = {
        "home": {
            ...
            "source_health": str(getattr(audit_home, "source_health", "ok") or "ok"),
            "source_links": int(getattr(audit_home, "source_links", 0) or 0),
            "source_api_ok": bool(getattr(audit_home, "source_api_ok", True)),
            "recovery_pass_used": bool(getattr(audit_home, "recovery_pass_used", False)),
        },
        "away": {
            ...
            "source_health": str(getattr(audit_away, "source_health", "ok") or "ok"),
            "source_links": int(getattr(audit_away, "source_links", 0) or 0),
            "source_api_ok": bool(getattr(audit_away, "source_api_ok", True)),
            "recovery_pass_used": bool(getattr(audit_away, "recovery_pass_used", False)),
        },
    }
            ...
            try:
                ...
                if (not history_only) and any(x in ff_vals for x in ("budget_exceeded_wait", "budget_exceeded")):
                    ...
                    for a in (a_home, a_away):
                        ...
                        absent_hits += int(exr.get("history_source_unavailable", 0) or 0)
                ...
                for a in (a_home, a_away):
                    ...
                    if int(exr.get("no_finished_singles", 0) or 0) > 0:
                        return "stats_absent"
                    if int(exr.get("history_source_unavailable", 0) or 0) > 0:
