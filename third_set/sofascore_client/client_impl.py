...
from third_set.sofascore_client.base import (
    SOFASCORE_API_BASE,
    SOFASCORE_TENNIS_LIVE_API,
    SOFASCORE_TENNIS_URL,
    SofascoreError,
    _normalize_sofascore_url,
    _sofascore_base_from_url,
    _tennis_url_from_page,
)
from third_set.sofascore_client.overlays import _dismiss_overlays_basic
...
async def _extract_player_profile_urls_with_diag(page: Page) -> Dict[str, Any]:
    try:
            ...
            () => {
              ...
              function dedup(list){
                ...
                for (const x of list) {
                  ...
                  const u = String(x.href).split('#')[0];
                  if (!u || seen.has(u)) continue;
                  seen.add(u);
                  out.push({href: u, text: String(x.text || '')});
                }
                return out;
              }
              const anchorsRaw = Array.from(document.querySelectorAll('a[href*="/tennis/player/"]')).filter(isVisible);
              const all = dedup(anchorsRaw.map(a => ({href: (a.href || '').split('#')[0], text: norm(a.innerText||a.textContent||'')})));
              const header = dedup(anchorsRaw.filter(inHeaderZone).map(a => ({href: (a.href || '').split('#')[0], text: norm(a.innerText||a.textContent||'')})));
              const selected = (header.length >= 2 ? header : all).slice(0, 2).map(x => x.href);
              const overlayVisible = !!(
                document.querySelector('[data-testid*="consent"], [id*="consent"], [class*="consent"], [class*="cookie"]') ||
                document.querySelector('button[aria-label*="Accept"], button[aria-label*="Принять"]')
              );
              return {
                selected,
                allVisibleCount: all.length,
                headerVisibleCount: header.length,
                topVisible: all.slice(0, 6),
                strategy: header.length >= 2 ? "header_anchors" : "all_visible_anchors",
                overlayVisible
              };
            }
            """
...
def _profile_urls_from_diag_payload(payload: Dict[str, Any]) -> Dict[str, str]:
    out: Dict[str, str] = {}
    selected = payload.get("selected") if isinstance(payload, dict) else None
    if isinstance(selected, list):
        if len(selected) >= 1 and isinstance(selected[0], str) and selected[0]:
            out["home"] = str(selected[0]).split("#", 1)[0]
        if len(selected) >= 2 and isinstance(selected[1], str) and selected[1]:
            out["away"] = str(selected[1]).split("#", 1)[0]
    if out.get("home") and out.get("away") and out.get("home") == out.get("away"):
        out.pop("away", None)
    if (not out.get("home")) or (not out.get("away")):
        top_visible = payload.get("topVisible") if isinstance(payload, dict) else None
        if isinstance(top_visible, list):
            links = [str(x.get("href") or "").split("#", 1)[0] for x in top_visible if isinstance(x, dict)]
...
async def discover_player_profile_urls_from_match_diagnostic(
    page: Page, *, match_url: str, event_id: int
    ...
    Returns: ({home/away urls}, diagnostic payload).
    """
    ...
    first_diag = await _extract_player_profile_urls_with_diag(page)
    out = _profile_urls_from_diag_payload(first_diag)
    retry_used = False
    if not out.get("home") or not out.get("away"):
        retry_used = True
        await page.wait_for_timeout(400)
        await _dismiss_overlays_basic(page)
        second_diag = await _extract_player_profile_urls_with_diag(page)
        second_out = _profile_urls_from_diag_payload(second_diag)
        if second_out.get("home") and second_out.get("away"):
            out = second_out
            first_diag = second_diag
        else:
            for k in ("home", "away"):
                if not out.get(k) and second_out.get(k):
                    out[k] = second_out[k]
            first_diag["retry"] = second_diag

    diag: Dict[str, Any] = {
        "strategy": first_diag.get("strategy"),
        "visible_anchors": int(first_diag.get("allVisibleCount") or 0),
        "header_anchors": int(first_diag.get("headerVisibleCount") or 0),
        "overlay_visible": first_diag.get("overlayVisible"),
        "retry_used": bool(retry_used),
        "resolved_home": bool(out.get("home")),
        "resolved_away": bool(out.get("away")),
    }
    return out, diag


async def discover_player_profile_urls_from_match(page: Page, *, match_url: str, event_id: int) -> Dict[str, str]:
    out, _diag = await discover_player_profile_urls_from_match_diagnostic(
        page, match_url=match_url, event_id=event_id
    )
    return out
...
async def get_team_last_events_via_navigation(
    page: Page, team_id: int, *, page_index: int = 0, timeout_ms: int = 15000
) -> Dict[str, Any]:
    """
    Fallback for /team/<id>/events/last/* when in-page fetch() is blocked.
    Uses navigation + response capture in the current browser context.
    """
    url_suffix = f"/api/v1/team/{int(team_id)}/events/last/{int(page_index)}"
    target = f"{SOFASCORE_API_BASE}/team/{int(team_id)}/events/last/{int(page_index)}"
    loop = asyncio.get_running_loop()
    fut: asyncio.Future[Dict[str, Any]] = loop.create_future()

    async def _on_response(resp: Response) -> None:
        try:
            if not resp.url.endswith(url_suffix):
                return
            txt = await resp.text()
            data = json.loads(txt)
            if not isinstance(data, dict):
                raise SofascoreError("Malformed team last events payload (navigation)")
            if not fut.done():
                fut.set_result(data)
        except Exception as exc:
            if not fut.done():
    ...
    page.on("response", _on_response)
    try:
        await _safe_goto(page, target, timeout_ms=timeout_ms)
        end = loop.time() + max(1.5, float(timeout_ms) / 1000.0)
        while loop.time() < end:
            if fut.done():
                break
            await asyncio.sleep(0.05)
        if not fut.done():
            raise SofascoreError("Timed out collecting: team_last_events")
        return fut.result()
    finally:
        page.remove_listener("response", _on_response)
